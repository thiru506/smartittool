...
if ($content) {
    list($title, $body) = $ost->replaceTemplateVariables(
        array($content->getName(), $content= __('To better serve you, we encourage our users to SignIn for an account')));
} else {
    $title = __('Sign In');
    $body = __('To better serve you, we encourage our users to SignIn for an account and verify the email address we have on record.');
}
...
<form action="login.php" method="post" id="clientLogin">
    ...
    <div style="display:table-row">
        <div class="login-box">
            ...
            <div>
                <input id="username" placeholder="<?php echo __('Enter Email id with srinivasa.co'); ?>" type="text" name="luser" size="30" value="<?php echo $email; ?>" class="nowarn">
            </div>
            <div>
