...
/*Adding menu items in Staff panel */
    function getTabs(){
        global $thisstaff;
...
/* Adding menu items in Admin panel*/
    function getTabs(){
...
/*Adding menu items at Client side*/
    function getNavLinks(){
        ...
        if(!$this->navs){
            ...
            $navs = array();
            $user = $this->user;
            $navs['home']=array('desc'=>__('SFPL Service Desk'),'href'=>'index.php','title'=>'');


            //$navs['kb']= array('desc'=> __( "HI") );
            //$ddArray2 = ARRAY('desc'=>'Red');
//$navs['kb'] = ARRAY('desc'=>'Red','desc'=>'green');
/* Array contents */
/* $navs['kb'] = array('s'=>'Sydney','m'=>'Melbourne','b'=>'Brisbane','T'=>'Tasmania','a'=>'Adelaide','p'=>'Perth','d'=>'Darwin','t'=>'ACT'); 
 
echo '<select name="cities">'; 
 
foreach($navs['kb'] as $cityid => $city) { 
...
echo '</select>'; */
		...
		$navs['new']=array('desc'=>__('How to create Request'),'href'=>'include/smartrequest.pdf','title'=>'');
            if($user && $user->isValid()) {
                if(!$user->isGuest()) {
                    $navs['tickets']=array('desc'=>sprintf(__('Tickets (%d)'),$user->getNumTickets($user->canSeeOrgTickets())),
                                           'href'=>'tickets.php',
                                            'title'=>__('Show all tickets'));
                    $navs['new']=array('desc'=>__('Open New Request'),'href'=>'open.php','title'=>'');
                    /*$navs['status']=array('desc'=>__('User Asset Info'),'href'=>"include/client/assetsdetails.php",'title'=>''); */
                } else {
		...
		$navs['status']=array('desc'=>__('How to reset Password'),'href'=>'include/How to reset Password.pdf','title'=>'');
